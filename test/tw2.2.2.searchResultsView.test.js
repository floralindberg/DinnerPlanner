...
describe("TW2.2.2 (array) rendering recap, fire custom events: SearchResultsView [test Vue](/tw2.2.2.html) [React](/tw2.2.2-react.html)", function tw2_2_2() {
  this.timeout(200000);
  ...
  //   function tw2_2_2_1(output, index){
  ...
  //   function tw2_2_2_2(output, index){
  ...
  testComponent({vue: SearchResultsView},
    ...
    function tw2_2_2_4(output, index){
  ...
  //   function tw2_2_2_3(output, index){
  //     const spans = output.queryAllByRole('img').map((x)=>{return x.parentElement});
  ...
  testComponent({vue: SearchResultsView},
    ...
    function tw2_2_2_5(output, index){
      const images = output.queryAllByRole('img');
  ...
  //   function tw2_2_2_6(output, index){
  ...
  it("SearchResultsView fires custom event on dish click, send search result as parameter", function tw2_2_2_7(){
    ...
    spans2.forEach((span, i) => {
      vueTL.fireEvent.click(span);
      expect(newDish?.id, "clicking a span should send correct dish as parameter").to.equal(searchResults[i].id);
      expect(newDish?.title, "clicking a span should send correct dish as parameter").to.equal(searchResults[i].title);
      newDish = undefined;
    });
  });

  it("SearchResultsView does not change its props during rendering", function  tw2_2_2_9() {
    if(!SearchResultsView)
        this.skip();
