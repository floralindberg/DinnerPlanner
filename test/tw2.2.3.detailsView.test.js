...
describe("TW2.2.3 Free rendering, fire custom events. Work with stacktraces: DetailsView [test Vue](/tw2.2.3.html) [React](/tw2.2.3-react.html)", function tw2_2_3() {
  this.timeout(200000);
  ...
  let disabled = true;
    ...
    testComponent({vue: DetailsView},
        ...
        function tw2_2_3_1(output, index){
        ...
        function tw2_2_3_2(output, index){
        ...
        function tw2_2_3_3(output, index){
        ...
        function tw2_2_3_4(output, index){
        ...
        function tw2_2_3_5(output, index){
        ...
        function tw2_2_3_5(output, index){
            ...
            expect(!!addToMenuButton.__vnode.props.disabled, "'Add to menu' button must be disabled if the dish is already in the menu").to.equal(disabled);
        });
    ...
    it("DetailsView 'add to menu' button fires a custom event", function tw2_3_20_7(){
        ...
        vueTL.fireEvent.click(button);
        expect(addedDish, "the add to menu button should fire a custom event").to.equal(true);
    });

    it("DetailsView does not change its props during rendering", function tw2_2_3_8() {
        if(!DetailsView)
            this.skip();
