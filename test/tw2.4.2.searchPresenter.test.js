...
const X = TEST_PREFIX;
const SearchPresenter= (await getModule(`/src/vuejs/${X}searchPresenter.jsx`))?.Search;
...
describe("TW2.4.2 Presenter passing props and handling custom events: Search [test Vue](/tw2.4.2.html)[React](/tw2.4.2-react.html)[Final TW2 test Vue](/vue.html#/search)[Final TW2 test React](/react.html#/search)", async function tw2_4_2() {
    this.timeout(200000);

    before(function(){
        if ((!SearchPresenter && !SearchPresenterReact) || !SearchFormView || !SearchResultsView) this.skip();
        if(!modelTemplate?.searchResultsPromiseState) this.skip();          
    });

    let searched, text, type, searchPars;
    beforeEach(function tw2_4_2_beforeEach1(){ searched=text=type=searchPars=undefined; });
    ...
    testComponent({
        ...
        "$framework Search presenter passes text, type and dishTypeOptions props to SearchFormView",
          function tw2_4_2_1(output, presenterProps, mockHandlers){
            ...
            expect(searchFormViewProps, "prop 'type' not passed to SearchFormView").to.have.property("type");
            expect(searchFormViewProps.type, "prop 'type' sent to SearchFormView should be read from the model search params").to.equal("test Type");
    ...
    //     async function tw2_4_2_2(output, presenterProps, mockHandlers){
    ...
    beforeEach(function tw2_4_2_beforeEach2(){ searched2=text2=type2=searchPars2= undefined; });
    if(hasParams)
    testComponent({
        vue: SearchPresenter,
        react: SearchPresenterReact,
        ...
        function tw2_4_2_3(output, presenterProps, mockHandlers){
            ...
            expect(searchFormViewProps[onSearchHandler].length, `custom event handler ${onSearchHandler} should have zero parameters`).to.equal(0);
    ...
    testComponent({
        ...
        mock: [{component: SearchFormView, dummyText: "mock searchForm view"}, {component: SearchResultsView, dummyText: "mock searchResults view"}]},
                  ...
                  function tw2_4_2_1(output, presenterPropsIndex, mockHandlers){
                      expect(output.container.firstChild?.tagName, "Search presenter must render a DIV").to.equal("DIV");
                      expect(output.container.firstChild.childNodes.length, "Search presenter DIV must contain two children: 1) the search form view and 2) suspense or search results view").to.equal(2);
                      expect(output.queryByText(/mock searchForm view/), "Search presenter should always render the search form view").to.be.ok;
                      ...
                      const msg= presenterPropsIndex===0 && "when there is no promise, Search presenter should show 'no data'"||
                            ...
                            presenterPropsIndex===2 && "when there is a promise but it rejected, Search presenter should render the error converted to string";
                      
                      expect(test, msg).to.be.ok
                  });
    
    let dishId;
    beforeEach(function tw2_4_2_beforeEach2(){ dishId= undefined; });
