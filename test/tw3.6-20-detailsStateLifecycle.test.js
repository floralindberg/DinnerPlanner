...
const X = TEST_PREFIX;
const DetailsPresenterVue= (await getModule(`/src/vuejs/${X}detailsPresenter.jsx`))?.Details;
...
describe("Bonus: current dish promise state in component state (remove model.currentDishPromiseState to enable) [test Vue](/tw2.4.1.html)[React](/tw2.4.1-react.html)", function tw3_6_20() {
    this.timeout(200000);
    before(function(){
        if(DetailsPresenterReact && DetailsView ||
           DetailsPresenterVue && DetailsView){
    ...
    async  function tw3_6_20_1(output, presenterPropsIndex, mockHandlers){
        ...
        withMyFetch(myDetailsFetch,
                    function() {
                        output.reactiveModel.currentDishId= dishInformation.id;
                    });
