...
describe("TW1.4 Handle native events, fire custom events [test events with Vue](/tw1.4.html)[or React](/tw1.4-react.html)", function tw1_4() {
    ...
    testComponent({vue: SidebarView},
        ...
        function tw1_4_2(output, index){
            ...
            output.fireEvent.click(buttons[0]);
            ...
            output.fireEvent.click(buttons[1]);
    ...
    testComponent({vue: SidebarView},
        ...
        function tw1_4_4(output, index){
            ...
            expect(currentParam1[0].title, "expected custom event "+currentName1+" fired on the first link to have the first dish as parameter").to.equal(dishInformation.title);
            expect(`id: ${currentParam1[0].id}`, "expected custom event "+currentName1+" fired on the first link to have the first dish as parameter").to.equal(`id: ${dishInformation.id}`);
            expect(currentParam2[0].title, "expected custom event "+currentName1+" fired on the second link to have the second dish as parameter").to.equal(dishes[1].title);
            expect(`id: ${currentParam2[0].id}`, "expected custom event "+currentName1+" fired on the second link to have the second dish as parameter").to.equal(`id: ${dishes[1].id}`);
