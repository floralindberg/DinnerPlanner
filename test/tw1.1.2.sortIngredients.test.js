...
describe("TW1.1.2 sort(CB) exercise: sortIngredients", function tw1_1_2() {
    ...
    it("compareIngredientsCB compares by aisle, then by name (export compareIngredientsCB and sortIngredients from utilities.js to enable)", function tw1_1_2_0(){
        ...
        expect(compareIngredientsCB(ingredientsConst[2], ingredientsConst[1]), "if first aisle is bigger than second aisle, reverse order (return a positive number)").to.be.gt(0);
        expect(compareIngredientsCB(ingredientsConst[1], ingredientsConst[2]), "if first aisle is smaller than second aisle, do not reverse order (return a negative number)").to.be.lt(0);
