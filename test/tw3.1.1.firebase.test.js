...
import {state, initDB} from "./mockFirebase.js";
...
function findPersistencePropNames(){
    ...
    const data= firebaseModel.modelToPersistence({numberOfGuests: 42, dishes:[{id:44}, {id:43}], currentDishId:45});
    ...
    expect(guests, "a property must exist for 'number of guests' and have the correct value").to.be.ok;
    expect(current, "a property must exist for 'current dish' and have the correct value").to.be.ok;
    expect(dishes, "a property must exist for 'dishes' and have the correct value").to.be.ok;
    ...
    expect(data[dishes], "dish IDs should always be saved in the same order, regardless of the order in the dish arrray").to.eql(firebaseModel.modelToPersistence({numberOfGuests: 42, dishes:[{id:43}, {id:44}], currentDishId:45})[dishes]);
    expect(firebaseModel.modelToPersistence({numberOfGuests: 42, dishes:[{id:16}, {id:10},  {id:25},  {id:35},  {id:12}], currentDishId:45})[dishes],
           "to avoid usesless persistence changes, dish IDs should always be saved in the same order, regardless of the order in the dish arrray").to.
            eql(firebaseModel.modelToPersistence({numberOfGuests: 42, dishes:[{id:12}, {id:35}, {id:16}, {id:25}, {id:10}], currentDishId:45})[dishes]);
    
    return {numberOfGuests: guests, dishes: dishes, currentDishId: current};
}

export async function tw3_1_1_3(){
...
describe("TW3.1.1 Firebase-model: Search test [Vue](/vue.html#/search)[React](/react.html#/search)", function tw3_1_1() {
    this.timeout(200000); // increase to allow debugging during the test run
    
    before(function tw3_1_1_before() {
    ...
    it("modelToPersistence", function tw3_1_1_1(){
        initDB(findPersistencePropNames());
    });

    it("saveToFirebase persists the result of modelToPersistence _only_ if model.ready is truthy", async function tw_3_1_1_2(){
    ...
    it("persistenceToModel sets model data and returns menu promise", async function tw3_1_1_1(){
        ...
        expect(model.dishes).to.be.an("array", "persistenceToModel populates the dishes array in the model");
    ...
    it("persistenceToModel when there are no dish IDs in the cloud", async function tw3_1_1_3(){
    ...
    it("persistenceToModel when there is no data persisted in the cloud", async function tw3_4_1_2(){
    ...
    it("readFromFirebase passes get() result to persistenceToModel and waits for it to resolve. Set model.ready to false during this process!", tw3_1_1_3);
    
    it("connectToFirebase", async function tw3_1_1_6(){
        ...
        expect(state.getHistory.length, "connectToFirebase reads the state from firebase to initialize the model (by calling get once)").to.equal(1);
        expect(model.currentDishId, "connectToFirebase reads the state from firebase to initialize the model").to.equal(state.data[currentDishId]);
    ...
    it("live update (optional): readFromFirebase invokes onValue() to copy cloud data into the model", async function tw3_1_1_3(){
        state.onHistory=[];
        state.getHistory=[];
        const model={setNumberOfGuests(g){this.numberOfGuests=g;}, setCurrentDishId(d){this.currentDishId=d;}};
        await withMyFetch(myDetailsFetch, async function(){
