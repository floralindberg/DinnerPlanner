...
describe("TW2.2.1 rendering recap, fire custom events: SearchFormView [test Vue](/tw2.2.1.html)[React](/tw2.2.1-react.html)", function tw2_2_1() {
  ...
  testComponent(
    ...
    function tw2_2_1_1(output, index){
  ...
  testComponent({vue: SearchFormView},
    ...
    function tw2_2_1_2_1(output, index){
      const options = output.queryAllByRole("option");
  ...
  it("SearchFormView fires custom events on text or type change (send text, type as parameter), and on button press", function tw2_2_1_3(){
          if(!SearchFormView)
      this.skip();
    ...
    const output= vueTL.render({
      ...
      props: {
        dishTypeOptions: ["apetizer", "main course", "dessert"],
    ...
    fireEvent.change(input, {target: {value: "some pizza"}});
    fireEvent.change(select, {target: {value: "apetizer"}});
    fireEvent.click(button);

    // Expects that the values inputted should be the same sent as props to presenter
    expect(newText, "the text box fires a custom event with the text content as parameter").to.equal("some pizza");
    expect(newType, "the SELECT box fires a custom event with the selected option as parameter").to.equal("apetizer");
    expect(searched, "the search button should fire a custom event").to.equal(true);
  });

    it("SearchFormView does not change its props during rendering", function tw2_2_1_5() {
      if(!SearchFormView)
          this.skip();
