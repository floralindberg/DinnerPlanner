...
const Details= (await getModule(`/src/reactjs/${X}detailsPresenter.jsx`))?.Details;
...
if(Details){
    const preamble=<div><p> This is the TW3.3 React Details presenter test (component state, lifecycle, observer, side effect)</p>
                     ...
                     <p>Edit tw/tw3.3.3.js to see other dishes</p>
                     <p>You can access and manipulate the model from the Console using myModel. Changing the model (e.g. setNumberOfGuests, setCurrentDishId) should be visible in the user interface.</p>
                 <hr/></div>;    
    ...
    setTimeout(function laterACB(){window.myModel.setCurrentDishId(AA);}, 2000);
    setTimeout(function laterACB(){window.myModel.setCurrentDishId(CC);}, 5000);
}
